    ...
    for (unsigned int i = 0; i < str.length(); i++)
    {
        ...
        else if (str[i] == '-' or str[i] == '+')
        {
            if (i == 0)
            {
                ...
                else // Encontrou um sinal de magnitude que não está no início da string
                    return false;
            }
            else
                return false;
            ...
            // Encontrou algo que não é um digito, nem um separador decimal e nem um
            // sinal de magnitude no ínicio da string
            return false;
    }
...
bool Parser::ParenthesisIsValid(std::string infix)
{
    ...
    // 2. Os parênteses devem estar ordenados, ou seja, uma expressão do tipo
    //    '1 + ) 3 - 5 (' não deve ser aceita como válida
    // 3. Dentro dos parênteses deve haver uma subexpressão válida, ou seja, algo do
    //    tipo '1 ( + ) 3' não deve ser aceito como válido

    std::string token;
    std::istringstream iss(infix);

    int32_t pendingClosingParenthesis = 0; // Usado rificação da premissa #1
    int32_t orderIsValid = 0; // Usada na verificação da premissa #2
...
bool Parser::InfixIsValid(std::string infix)
{
    ...
    if (not Parser::ParenthesisIsValid(infix))
        return false;
    ...
    while (iss >> token)
    {
        if (Parser::IsNumber(token))
        {
            ...
            if (not lastIsNumber) // Verificação da premissa #1
            {
                lastIsNumber = true;
                continue;
            }
        ...
        else if (Parser::IsValidOperator(token))
        {
            ...
            // Expressão contém um caractere que não compõem um número válido, nem é
            // um operador válido e não é parênteses
            return false;
    }
    if (not lastIsNumber) // Verificação das premissas #4
        return false;
...
bool Parser::PostfixIsValid(std::string postfix)
{
    ...
    // 1. A expressão não pode começar com um operador, além disso os dois primeiros
    //    tokens da string necessariamente são operandos
